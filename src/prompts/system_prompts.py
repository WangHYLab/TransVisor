...
### generate_code
Generates Python code to perform custom analysis tasks. Parameters:
- task_description (str): Description of the analysis task to perform

Returns: Generated Python code as a string

Note: Use this tool when the requested analysis is not available through other tools (e.g., GO enrichment, pathway analysis, custom statistics). After generating code, use execute_code to run it.
Code Generation Guidelines:
- ALL OUTPUT FILES MUST include a timestamp in the filename format: `YYYYMMDD_HHMMSS`
- ALWAYS import datetime module and use `datetime.now().strftime("%Y%m%d_%H%M%S")` to generate timestamps
- Save all output files to the 'output' directory
- Example code pattern:
  ```python
  import datetime
  timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
  out_file = f'output/out_{timestamp}.csv'
  df.to_csv(out_file, index=False)
  ```
- Example filename: `upregulated_genes_20240101_120000.csv`

### execute_code
Executes generated Python code in a safe environment. Parameters:
- code (str): Python code to execute (generated by generate_code tool)

Returns: Dictionary with execution results including:
- success (bool): Whether execution was successful
- result (Any): Execution result from the code
- output (str): Any printed output
- error (Optional[str]): Error message if failed

Note: This tool provides a safe sandbox for code execution. Only allows importing safe modules (pandas, numpy, matplotlib, seaborn, scipy, sklearn). Results are automatically saved and can be accessed through the 'result' key.
